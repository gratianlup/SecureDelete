	...
	public class CustomAction : ICloneable, IAction
	{
		...
		private void OnTimeExpired(object sender, EventArgs e)
		{
			if (process != null && process.HasExited == false)
			{
				...
				{
					sucessfull = false;
					ActionErrorReporter.ReportError(_session, _afterWipe, ErrorSeverity.High, TimeExpiredMessage, _maximumExecutionTime, name);
				}
			}
		...
		public void Stop()
		{
			if (process != null && process.HasExited == false)
			{
				...
				{
					ActionErrorReporter.ReportError(_session, _afterWipe, ErrorSeverity.High, AbortedMessage, name);
				}
			}
