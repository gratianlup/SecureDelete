...
namespace SecureDelete
{
	...
	public class WipeContext
	{
		...
		/// </summary>
		public bool GetFailedObject(int index, out NativeMethods.WSmallObject item,bool getAssociatedError,out NativeMethods.WError error)
		{
			CheckContextInitialized();
			...
			if (result == NativeMethods.ERRORCODE_SUCCESS && getAssociatedError == true)
			{
		...
		public KeyValuePair<NativeMethods.WSmallObject, NativeMethods.WError>[] GetFailedObjectsWithErrors()
		{
			...
			// get the errors
			List<KeyValuePair<NativeMethods.WSmallObject, NativeMethods.WError>> objects = new List<KeyValuePair<NativeMethods.WSmallObject, NativeMethods.WError>>();
			...
			for (int i = 0; i < number; i++)
			{
				...
				{
					KeyValuePair<NativeMethods.WSmallObject, NativeMethods.WError> pair = new KeyValuePair<NativeMethods.WSmallObject, NativeMethods.WError>(item,error);
